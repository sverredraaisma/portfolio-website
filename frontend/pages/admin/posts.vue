...
<template>
  <section class="max-w-4xl mx-auto px-6 py-10">
    <div class="flex items-center justify-between mb-6">
      <h1 class="text-2xl text-green-400">$ admin/posts</h1>
      <NuxtLink to="/posts/new" class="text-sm bg-green-600 hover:bg-green-500 text-black font-bold rounded px-3 py-1">
        + new
      </NuxtLink>
    </div>
    ...
    <ul v-else class="divide-y divide-zinc-800 border border-zinc-800 rounded">
      <li
        v-for="p in posts"
        :key="p.id"
      ...
      >
        <div class="flex-1 min-w-0">
          <NuxtLink :to="`/posts/${p.slug}`" class="block truncate hover:text-green-400">
            {{ p.title }}
          </NuxtLink>
          <div class="text-xs text-zinc-500 truncate">
            <span :class="p.published ? 'text-green-500' : 'text-yellow-500'">
        ...
        <button
          ...
          class="text-xs px-2 py-1 rounded border border-zinc-700 hover:border-zinc-500 disabled:opacity-50"
        >
        ...
        <NuxtLink
          ...
          class="text-xs px-2 py-1 rounded border border-zinc-700 hover:border-zinc-500"
        >
          edit
        </NuxtLink>
        ...
        <button
          :disabled="busyId === p.id"
          @click="remove(p)"
          class="text-xs px-2 py-1 rounded border border-red-900 text-red-400 hover:border-red-700 disabled:opacity-50"
        >
          ✕
        </button>
      </li>
